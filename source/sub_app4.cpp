...
Thread sapp4_init_thread, sapp4_exit_thread, sapp4_worker_thread, sapp4_hid_thread;
...
void Sapp4_hid_thread(void* arg)
{
	Util_log_save(DEF_SAPP4_HID_THREAD_STR, "Thread started.");
	Hid_info key;

	while (sapp4_thread_run)
	{
		Util_hid_query_key_state(&key);
		if (sapp4_main_run && var_previous_ts != key.ts)
		{
	...
	Util_log_save(DEF_SAPP4_HID_THREAD_STR, "Thread exit.");
	threadExit(0);
}
...
void Sapp4_init_thread(void* arg)
{
	...
	sapp4_hid_thread = threadCreate(Sapp4_hid_thread, (void*)(""), 1024 * 4, DEF_THREAD_PRIORITY_REALTIME, 0, false);
...
void Sapp4_exit_thread(void* arg)
{
	...
	Util_log_save(DEF_SAPP4_EXIT_STR, "threadJoin()...", threadJoin(sapp4_hid_thread, DEF_THREAD_WAIT_TIME));
