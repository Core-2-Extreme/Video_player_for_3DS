...
Thread vid_init_thread, vid_exit_thread, vid_decode_thread, vid_decode_video_thread, vid_convert_thread, vid_read_packet_thread;
...
Large_image vid_large_image[DEF_VID_BUFFERS][2];
		...
		else
		{
			if(vid_menu_mode == DEF_VID_MENU_SETTINGS_0)
			{
				...
				else if(Util_hid_is_released(key, vid_texture_filter_button) && vid_texture_filter_button.selected)
				{
					...
					for(int i = 0; i < DEF_VID_BUFFERS; i++)
					{
						for(int k = 0; k < 2; k++)
							Vid_large_texture_set_filter(&vid_large_image[i][k], vid_linear_filter);
					}
				}
		...
		if(vid_play_request || vid_change_video_request)
		{
			...
			if(num_of_video_tracks > 0 && vid_play_request)
			{
				...
				if(vid_play_request && num_of_video_tracks > 0)
				{
					...
					//Allocate tecture buffers.
					for(int i = 0; i < DEF_VID_BUFFERS; i++)
					{
						for(int k = 0; k < loop; k++)
						{
							Vid_large_texture_init(&vid_large_image[i][k], vid_codec_width, vid_codec_height, PIXEL_FORMAT_RGB565LE, true);
							if(result.code != 0)
							{
								i = DEF_VID_BUFFERS;
			...
			{
				for(int k = 0; k < 2; k++)
					Vid_large_texture_free(&vid_large_image[i][k]);
			}
				...
				else if(vid_num_of_video_tracks > 0)
				{
					...
					if(result.code == 0)
					{
						...
						if(result.code == 0)
						{
							...
							if(!vid_hw_decoding_mode && vid_hw_color_conversion_mode)//Raw image is texture format.
								result = Vid_large_texture_set_data(&vid_large_image[image_num][packet_index], video, vid_codec_width, vid_codec_height, true);
							else//Raw image is NOT texture format.
								result = Vid_large_texture_set_data(&vid_large_image[image_num][packet_index], video, vid_codec_width, vid_codec_height, false);
...
void Vid_main(void)
{
	...
	image_width = vid_codec_width * (vid_correct_aspect_ratio_mode ? vid_video_info.sar_width : 1) * vid_video_zoom;
	image_height = vid_codec_height * (vid_correct_aspect_ratio_mode ? vid_video_info.sar_height : 1) * vid_video_zoom;
	...
	if(Util_is_watch_changed() || var_need_reflesh || !var_eco_mode)
	{
		...
		if(var_turn_on_top_lcd)
		{
			...
			{
				//video
				Vid_large_texture_draw(&vid_large_image[image_num][0], vid_video_x_offset, vid_video_y_offset, image_width, image_height);
			...
			if(Draw_is_3d_mode())
			{
				...
				{
					//3d video (right eye)
					Vid_large_texture_draw(&vid_large_image[image_num_3d][1], vid_video_x_offset, vid_video_y_offset, image_width, image_height);
		...
		if(var_turn_on_bottom_lcd)
		{
			...
			else
			{
				...
				{
					//video
					Vid_large_texture_draw(&vid_large_image[image_num][0], vid_video_x_offset - 40, vid_video_y_offset - 240, image_width, image_height);
				...
				else if(vid_menu_mode == DEF_VID_MENU_INFO)
				{
					...
					{
						for(int i = 0; i < 319; i++)
							Draw_line(i, y_offset - (vid_time[1][i] / 2) + vid_ui_y_offset, DEF_DRAW_BLUE, i + 1, y_offset - (vid_time[1][i + 1] / 2) + vid_ui_y_offset, DEF_DRAW_BLUE, 1);//Thread 1
					}
					//decoding time
					if(vid_show_decode_graph_mode)
					{
						for(int i = 0; i < 319; i++)
							Draw_line(i, y_offset - (vid_time[0][i] / 2) + vid_ui_y_offset, DEF_DRAW_RED, i + 1, y_offset - (vid_time[0][i + 1] / 2) + vid_ui_y_offset, DEF_DRAW_RED, 1);//Thread 0		
					...
					{
						for(int i = 0; i < 319; i++)
							Draw_line(i, y_offset - vid_packet_buffer[i] / 3.0 + vid_ui_y_offset, 0xFFFF00FF, i + 1, y_offset - vid_packet_buffer[i + 1] / 3.0 + vid_ui_y_offset, 0xFFFF00FF, 1);//Packet buffer
					}
					//raw video buffer
					if(vid_show_raw_video_buffer_graph_mode)
					{
						for(int i = 0; i < 319; i++)
							Draw_line(i, y_offset - vid_raw_video_buffer[i] / 1.5 + vid_ui_y_offset, 0xFF2060FF, i + 1, y_offset - vid_raw_video_buffer[i + 1] / 1.5 + vid_ui_y_offset, 0xFF2060FF, 1);//Raw image buffer
					}
					//raw audio buffer
					if(vid_show_raw_audio_buffer_graph_mode)
					{
						for(int i = 0; i < 319; i++)
							Draw_line(i, y_offset - vid_raw_audio_buffer[i] / 6.0 + vid_ui_y_offset, 0xFF00A000, i + 1, y_offset - vid_raw_audio_buffer[i + 1] / 6.0 + vid_ui_y_offset, 0xFF00A000, 1);//Raw audio buffer
					}

					//bottom line
					Draw_line(0, y_offset + vid_ui_y_offset, color, 320, y_offset + vid_ui_y_offset, color, 1);
					//deadline
					Draw_line(0, y_offset + vid_ui_y_offset - (vid_frametime / 2), 0xFFD0D000, 320, y_offset - (vid_frametime / 2) + vid_ui_y_offset, 0xFFD0D000, 1);
